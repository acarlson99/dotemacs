...
(defun c++-insert-exception-src (name nest)
  ...
  (if (string= nest "")
	  (progn
		(insert name"Exception::"name"Exception( void ) { }\n")
		(insert name"Exception::"name"Exception( "name"Exception const & cp) { *this = cp; }\n")
		(insert name"Exception::~"name"Exception( void ) throw() { }\n")
		(insert name"Exception& "name"Exception::operator=( "name"Exception const &) { return *this; }\n")
		(insert "const char* "name"Exception::what( void ) const throw() {\n\treturn \"ERR_MSG\";\n}\n")
		)
	(progn
